...
const submit = () => {
  movieShowForm.post(route('shows.edit', props.show.id), {
    onSuccess: () => {
      success.value = true
    }
  })
...
onBeforeMount(() => {
  movieShowForm.available = props.show.available == 1
  movieShowForm.price = props.show.price
  movieShowForm.movie_id = props.show.movie.id
  movieShowForm.room_id = props.show.room.id
  movieShowForm.starts_at = new Date().toISOString().slice(0, 16).replace(' ', 'T');
})
